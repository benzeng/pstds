...
def create_candlestick_chart(
    ...
    if show_macd and 'MACD' in df.columns:
        fig.add_trace(
            ...
            row=3 if show_rsi else 3 if show_macd else None, col=1,
        )
    ...
    if show_rsi and 'RSI' in df.columns:
        ...
        fig.add_trace(
            ...
            row=4 if show_macd and show_rsi else None, col=1,
        )
    ...
    if show_macd:
        fig.update_yaxes(title_text="MACD", row=3 if show_rsi else 3, col=1)

    if show_rsi:
        fig.update_yaxes(title_text="RSI", row=4 if show_macd else 3, col=1)
        fig.update_yaxes(range=[0, 100], row=4 if show_macd else 3, col=1)
    ...
    if PANDAS_TA_AVAILABLE:
        ...
        if len(df) >= 26:
            ...
            if macd_df is not None and not macd_df.empty:
                # 获取实际的列名（可能因版本不同而变化）
                macd_cols = [c for c in macd_df.columns if 'MACD' in c]
                if macd_cols:
                    df['MACD'] = macd_df[macd_cols[0]]
                signal_cols = [c for c in macd_df.columns if 'signal' in c.lower()]
                if signal_cols:
                    df['MACD_signal'] = macd_df[signal_cols[0]]
                hist_cols = [c for c in macd_df.columns if 'hist' in c.lower() or 's_' in c]
                if hist_cols:
